...
function post_gic_opf_ts(pm::GenericPowerModel)
    for (n, network) in nws(pm)
        PowerModels.variable_voltage(pm, nw=n) #OK
        ...
        PowerModels.variable_generation(pm, nw=n) #OK
        PowerModels.variable_branch_flow(pm, nw=n) #OK
        PowerModels.variable_dcline_flow(pm, nw=n) #OK
        PowerModelsGMD.variable_dc_line_flow(pm, nw=n) #OK

        variable_delta_oil_ss(pm, nw=n)
        variable_delta_oil(pm, nw=n)

        PowerModels.constraint_model_voltage(pm, nw=n) #OK

        for i in PowerModels.ids(pm, :ref_buses, nw=n)
            PowerModels.constraint_theta_ref(pm, i, nw=n) #OK
        end

        for i in PowerModels.ids(pm, :bus, nw=n)
            PowerModelsGMD.constraint_kcl_gmd(pm, i, nw=n) #OK
        end
        ...
        for i in PowerModels.ids(pm, :branch, nw=n)
            ...
            PowerModels.constraint_ohms_yt_from(pm, i, nw=n) #OK
            PowerModels.constraint_ohms_yt_to(pm, i, nw=n) #OK

            PowerModels.constraint_voltage_angle_difference(pm, i, nw=n) #OK

            PowerModels.constraint_thermal_limit_from(pm, i, nw=n) #OK
            PowerModels.constraint_thermal_limit_to(pm, i, nw=n) #OK
        ...
        for i in PowerModels.ids(pm, :dcline, nw=n)
            PowerModels.constraint_dcline(pm, i, nw=n) #OK
        end
    end

    PowerModelsGMD.objective_gmd_min_fuel(pm) #OK
end
...
run_gic_opf_ts(net, PowerModelsGMD.ACPPowerModel, ipopt_solver)
